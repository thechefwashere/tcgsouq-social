...
| D8 | **Ops hub at `hub.tcgsouq.com`** — projects, connections, health, logs and a timeline in one app; business-metrics dashboard cut; Supabase Auth, not Cloudflare Access | **Decided** — 19 Sep 2026. Spec in `docs/hub/OPS-HUB.md`. Cutting the metrics half takes every external approval off the critical path |
