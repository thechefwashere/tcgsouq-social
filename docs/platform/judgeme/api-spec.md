---
title: "Judge.me OpenAPI spec (YAML)"
source: "https://judge.me/api/docs.yaml"
final_url: "https://judge.me/api/docs.yaml"
platform: "judgeme"
...
http_status: "200"
format: "raw"
sha256: "a490f65ed314d2dc2b26b92e17da7d42c81097aafbe77d7f4107557e406e850c"
note: "Saved verbatim; not converted"
---
...
info:
  ...
  description: |
    ...
    # Authentication
    Judge.me accepts two kinds of credentials, and they are passed differently:

    - **API keys** (private/public) — pass in the `X-Api-Token` HTTP header (recommended):
    ```
    ...
    ```
    - **OAuth access tokens** — pass in the `Authorization: Bearer` header (see the OAuth section below). The `X-Api-Token` header is **not** supported for OAuth access tokens.

    ## OAuth
    [Judge.me](http://judge.me/) uses OAuth2 to grant App Developers access to [Judge.me](http://judge.me/) API. Follow [this guide](https://help.judge.me/en/articles/8283047-setting-up-the-oauth-flow-for-your-app-in-judge-me) to set up the OAuth flow for your app.
    ...
    ```
    Once you have exchanged the code for an access token, authenticate your API calls with the `Authorization: Bearer` header:
    ```
    curl -H "Authorization: Bearer YOUR_ACCESS_TOKEN" "https://api.judge.me/api/v1/reviews?shop_domain=example.myshopify.com"
    ```
    The access token may also be passed as an `api_token` query parameter. Do **not** use the `X-Api-Token` header for OAuth access tokens — it is only recognised for private/public API keys, and an OAuth token sent that way fails with `Failed to authenticate. Shop domain or Api Token is wrong`. With OAuth the shop is derived from the token, so `shop_domain` is not required for authentication.
...
components:
  ...
  securitySchemes:
    PrivateAPIKey:
      ...
      in: header
      name: X-Api-Token
      description: "Private API key. Pass as X-Api-Token header."
    PublicAPIKey:
      type: apiKey
      in: header
      name: X-Api-Token
      description: "Public API key. Pass as X-Api-Token header."
    ShopDomain:
      type: apiKey
      in: query
      name: shop_domain
    OAuthAPIKey:
      type: http
      scheme: bearer
      description: "OAuth access token. Pass in the Authorization header as 'Bearer <token>', or as an api_token query parameter. The X-Api-Token header is NOT supported for OAuth access tokens."
  examples:
    datetime_str:
